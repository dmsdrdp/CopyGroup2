    ...
    public class CopyGroup : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                Room roomSelected = GetRoomByPoint(doc, point); //определение комнаты по точке
                XYZ selectedRoomCenter = GetElementCenter(roomSelected); // центр комнаты
                XYZ offsetForSelectedRoom = offset + selectedRoomCenter;  // смещение для вставляемой группы      

                Transaction transaction = new Transaction(doc);
                transaction.Start("Копирование группы объектов");

                doc.Create.PlaceGroup(offsetForSelectedRoom, group.GroupType); 
